...
;; This module contains logging facilities.


;;; Code:

(define-module (web driver log)
...
(define-with-docs %default-port-log-file
  ...
  "/var/log/guile-web-driver-ng.log")

(define-with-docs %default-syslog-tag
  "The default syslog tag used for the project."
  "guile-web-driver-ng")
...
(define-class-with-docs <precise-logger> (<logger>)
  "Precise logger that prints log with microsecond accuracy.")
...
(define-method (accept-log (log <null-log>) level time str)
...
(define-method (accept-log (log <system-log>) level time str)
  (let* ((command (format #f "~a -p 'user.~a' -t '~a' '~a'"
    ...
    (unless (zero? result)
      (format (current-error-port) "ERROR: Could not log a message"))))
...
(define-method (log-init! (driver <string>) (options <list>))
  ...
  (cond
   ((string=? driver "syslog")
    (log-add-handler! (make <system-log> #:tag (or (assoc-ref options 'tag)
                                                   %default-syslog-tag))))
   ((string=? driver "file")
...
(define (log-warning fmt . args)
  ...
  (apply log-message 'WARNING fmt args))
